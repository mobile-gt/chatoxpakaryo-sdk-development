...
public class AutoLabelUI extends AutoViewGroup implements Label.OnClickCrossListener, Label.OnLabelClickListener {
    ...
    private void getAttributes(AttributeSet attrs) {
        ...
        if (typedArray != null) {
            try {
                mTextSize = typedArray.getDimensionPixelSize(R.styleable.LabelsView_txt_size,
                        getResources().getDimensionPixelSize(R.dimen.dimen_10dp));
                mTextColor = typedArray.getColor(R.styleable.LabelsView_txt_color,
                        getResources().getColor(android.R.color.white));
                mBackgroundResource = typedArray.getResourceId(R.styleable.LabelsView_label_background_res,
//                        R.color.grey_400);
                        R.drawable.bg_square_corner_black);
     ...
     *
     * @param model is the text of the label added using a LIST.
     * @param position  is the position of the label.
     */
    public boolean addLabel(AutoLabelUiModel model, int position) {
     ...
     * Method to add a Label if is possible.
     *
     * @param model is the text of the label added.
     */
    public boolean addLabel(AutoLabelUiModel model) {
        if (!checkLabelsCompleted()) {
            Label label = new Label(getContext(), mTextSize, mIconCross, mShowCross,
                    mTextColor, mBackgroundResource, mLabelsClickables, mLabelPadding);
            label.setLayoutParams(new LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
    ...
    private List<LabelValues> getAllLabelsAdded() {
        ...
        for (int i = 0; i < childcount; i++) {
            ...
            if (label.getTag() instanceof Integer) {
                listLabelValues.add(new LabelValues((int) label.getTag(), label.getText(), label.getAutoLabelModel()));
            } else {
                listLabelValues.add(new LabelValues(-1, label.getText(), label.getAutoLabelModel()));
            }
        }
            ...
            if (labelsAdded != null) {
                for (int i = 0; i < labelsAdded.size(); i++) {
                    ...
                    if (labelValues.getKey() == -1) {
                        addLabel(labelValues.getAutoLabelModel());
                    } else {
                        addLabel(labelValues.getAutoLabelModel(), labelValues.getKey());
                    }
                }
            }
